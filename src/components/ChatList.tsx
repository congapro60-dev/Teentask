  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-24">
      ...
      <div className="bg-slate-900/50 p-8 pb-12 rounded-b-[64px] border-b border-white/5 sticky top-0 z-30 backdrop-blur-2xl">
        ...
        <div className="absolute top-20 left-0 w-48 h-48 bg-secondary/10 rounded-full blur-[80px] -z-10"></div>

        <div className="flex items-center gap-3 mb-2">
          <div className="w-2 h-5 bg-primary rounded-full shadow-[0_0_15px_rgba(79,70,229,0.5)]"></div>
          <span className="text-[11px] font-black text-slate-500 uppercase tracking-[0.3em]">Kết nối & Trao đổi</span>
        </div>
        <h1 className="text-4xl font-black tracking-tighter text-white mb-10">Tin nhắn</h1>
        
        <div className="relative group">
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-primary transition-colors" size={22} strokeWidth={3} />
          <input
            type="text"
            placeholder="Tìm kiếm cuộc trò chuyện..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-16 pr-8 py-6 bg-white/5 border-2 border-transparent rounded-[32px] text-sm font-bold text-white placeholder:text-slate-600 focus:bg-white/10 focus:border-primary/20 focus:ring-8 focus:ring-primary/5 outline-none transition-all shadow-inner"
          />
        </div>
      </div>
      ...
      <div className="px-8 py-10 space-y-6">
        {displayChats.length > 0 ? (
            ...
            return (
              ...
              >
                ...
                <div className={`absolute -top-10 -right-10 w-32 h-32 rounded-full blur-3xl transition-colors ${
                  isBot ? 'bg-primary/20' : 'bg-primary/5 group-hover:bg-primary/10'
                }`}></div>

                <div className="relative z-10">
                  <div className={`w-20 h-20 rounded-[32px] flex items-center justify-center overflow-hidden border-4 shadow-2xl group-hover:scale-110 transition-transform duration-500 ${
                ...
                <div className="flex-1 min-w-0 relative z-10">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-3">
                      <h3 className="font-black text-white text-lg tracking-tight truncate group-hover:text-primary transition-colors">{otherParticipant?.displayName}</h3>
                      {isBot && (
                    ...
                    {chat.lastMessageAt && (
                      <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest whitespace-nowrap pt-1">
                        {formatDistanceToNow(chat.lastMessageAt, { addSuffix: false, locale: vi })}
                      </span>
                    )}
                  </div>
                  <p className="text-sm text-slate-500 truncate font-bold leading-relaxed mb-4">
                    {chat.lastMessage || 'Bắt đầu cuộc trò chuyện mới'}
                  </p>
                  {chat.relatedTo && (
                    <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 rounded-2xl border border-white/5 group-hover:bg-primary/10 group-hover:border-primary/10 transition-colors">
                      <Sparkles size={12} className="text-primary" strokeWidth={3} />
                      <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest truncate max-w-[150px] group-hover:text-primary transition-colors">
                ...
                <div className={`w-12 h-12 rounded-[20px] flex items-center justify-center transition-all shadow-2xl relative z-10 border ${
                  isBot 
                    ? 'bg-primary text-white border-primary/20' 
                    : 'bg-white/5 text-slate-600 border-white/5 group-hover:bg-primary group-hover:text-white'
                }`}>
          ...
          <div className="flex flex-col items-center justify-center py-32 text-slate-600">
            ...
            <p className="text-[10px] font-bold mt-3 uppercase tracking-widest text-slate-700">Hãy bắt đầu kết nối với các doanh nghiệp!</p>
          </div>
        )}
      </div>
    </div>
